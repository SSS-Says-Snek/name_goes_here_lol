...
class ShopEntity(BaseEntity):
    def __init__(self, *args, **kwargs):
        ...
        self.start_shop_pos = [random.randint(0, common.WIDTH), random.randint(0, common.HEIGHT)]
        self.shop_pos = self.start_shop_pos[:]

        self.text_message = TextMessage((20, 400), 760, 190, (128, 128, 128),
                                        "Are you sure you want to enter the shop? (Enter to enter shop, move away to exit)",
                                        utils.font(50), instant_blit=False)
        self.draw_text_message = False

    def draw(self):
        pygame.draw.rect(self.screen, (0, 0, 128), [self.shop_pos[0],
                                                    self.shop_pos[1], 60, 60])
    ...
    def constant_run(self):
        ...
        if pygame.Rect(
                ...
                game_data.player.y1 - game_data.camera_offset[1], 20, 20
        ).colliderect(
            pygame.Rect(self.shop_pos[0],
                        self.shop_pos[1], 60, 60)
        ):
